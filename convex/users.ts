...
export const createUser = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const userId = await ctx.db.insert('users', { email: args.email, name: args.name, clerkId: args.clerkId, stripeCustomerId: args.stripeCustomerId })
        return userId;
...
export const getUserByStripeCustomerId = query({
    ...
    handler: async (ctx, args) => {
        return ctx.db.query('users').withIndex('by_stripeCustomerId', (q) => q.eq('stripeCustomerId', args.stripeCustomerId)).unique()
    }
})
...
export const getUserAccess = query({
    ...
    handler: async (ctx, args) => {
        ...
        const purchase = await ctx.db.query("purchases")
            .withIndex('bu_userId_and_courseId', (q) => q.eq("userId", args.userId).eq('courseId', args.courseId))
            .unique()
